...
function get_CamelCase_identifier(words_num) {
    ...
    for (var i = 1; i <= words_num; i++) {
        task += capitalize_first_letter(words[get_random_int(0, words.length)]);
    }
...
function get_SnakeCase_identifier(words_num) {
    ...
    for (var i = 1; i <= words_num; i++) {
        if (i == words_num) {
            task += words[get_random_int(0, words.length)];
        } else {
            task += words[get_random_int(0, words.length)] + "_";
        }
...
document.experiment_definition(
    {
        ...
        task_configuration: (t) => {
            ...
            if (t.treatment_combination[0].value == "CamelCase")
                t.code = get_CamelCase_identifier(t.expected_answer);
            else
                t.code = get_SnakeCase_identifier(t.expected_answer);
